...
class Amazon(BaseService):
	...
	def get_tracks(self, title: Title) -> Tracks:
		...
		if need_uhd_audio and (self.config.get("device") or {}).get(self.profile, None):
			...
			uhd_audio_manifest = self.get_manifest(
				...
				manifest_type="SmoothStreaming", # or "DASH"
				ignore_errors=True
			)
	...
	def configure(self) -> None:
		...
		if (self.quality > 1080 or self.range != "SDR" or self.atmos):
	...
	def get_best_quality(self, title):
		...
		for bitrate in bitrates:
			...
			try:
				try:
					mpd_url = self.clean_mpd_url(chosen_manifest["avUrlInfoList"][1]["url"], optimise=False)
				except:
					mpd_url = self.clean_mpd_url(chosen_manifest["avUrlInfoList"][1]["url"], optimise=False)
			except:
				chosen_manifest = default_url_set["urls"]["manifest"]
	...
	class DeviceRegistration:
		...
		def refresh(self, device: dict, refresh_token: str, access_token: str) -> dict:
			...
			if response.status_code != 200 or "error" in response_json:
				...
				raise self.log.exit(f"Failed to refresh device token -> {response_json['error_description']} [{response_json['error']}]")
			self.log.debug(response_json)
