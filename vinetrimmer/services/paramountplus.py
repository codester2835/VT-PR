...
class ParamountPlus(BaseService):
	...
	def get_titles(self):
		if self.movie:
			...
			config = next((x.get("videoConfigUniqueName") for x in links if x.get("title").strip() in ["Episodes", "Match Replays"]), None)
			...
			section = next(
				(x["sectionId"] for x in show_data["videoSectionMetadata"] if x["title"] in ["Full Episodes", "Match Replays"]), None
			)
	...
	def get_tracks(self, title: Title):
		...
		self.log.warning(f" + Downloading Manifest ---> {mpd_url}")
